...
For each of the 8 model slots (3 WEIRD + 3 Sinic + 2 bilingual), embeds all
N terms using the glossary-cleaned surface form:
  - WEIRD (lang=en) / bilingual EN slot: term["en_clean"]   (fallback term["en"])
  - Sinic (lang=zh) / bilingual ZH slot: term["zh_clean"]   (fallback term["zh_canonical"])

The cleaned forms strip DOJ-glossary metadata (parens, markers `※☛`, brackets
`〔〕`, terminal `的` adverbial suffix) per D1 of trace_firthian_pivot.md.
The original `en` and `zh_canonical` are preserved in legal_terms.json and
in the index.json for citation; encoding uses the cleaned forms only.
...
def build_index(terms: list[dict]) -> list[dict]:
    """Extract the per-term metadata used by Lens experiments and downstream
    scripts. Includes both original (en, zh_canonical) and cleaned (en_clean,
    zh_clean) surface forms; cleaned forms are the canonical encoding input
    after D1 of trace_firthian_pivot.md."""
    return [
        {
            "en": t["en"],
...
def main(args: argparse.Namespace) -> int:
    ...
    # Use glossary-cleaned forms for encoding (D1 of trace_firthian_pivot.md);
    # fall back to original fields if `*_clean` is missing for legacy datasets.
    en_texts = [t.get("en_clean") or t["en"] for t in terms]
    zh_texts = [t.get("zh_clean") or t["zh_canonical"] for t in terms]
