...
exact match for ZH) using the glossary-cleaned forms `en_clean` /
`zh_clean` for matching. The original `en` and `zh_canonical` are
preserved in the output record for citation. Extracts a context window
around each occurrence.

Matching forms (post-D1 of trace_firthian_pivot.md):
- EN matching:  en_clean (split on `;`, otherwise verbatim)
- ZH matching:  zh_clean (parens, markers `※☛`, brackets `〔〕`, terminal `的` stripped)
...
def main(args: argparse.Namespace) -> int:
    ...
    for i, t in enumerate(terms):
        # Match against en_clean (post-D1 cleanup), fall back to en if absent
        en = t.get("en_clean") or t["en"]
        en_lower = en.lower()
    ...
    for i, t in enumerate(terms):
        # Match against zh_clean (post-D1 cleanup), fall back to zh_canonical
        ...
        for i, t in enumerate(terms):
            ...
            if en_ctxs or zh_ctxs:
                ...
                record = {
                    ...
                    "term_zh_match": t.get("zh_clean") or t["zh_canonical"],
                    "domain": t.get("domain"),
                    "tier": t["tier"],
                    "en_contexts": en_ctxs,
                    "zh_contexts": zh_ctxs,
